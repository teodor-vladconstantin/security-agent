...
- Task 7 (open the real demo PR, confirm a comment appears on the vulnerable one and silence on the clean one): **still blocked**, not started — now on a second, independent quota problem:
  - Bedrock: unchanged, still returns `ThrottlingException: Too many tokens per day` on every call. AWS Support case still open, Basic support tier, no SLA.
  - Gemini (the already-built `MODEL_PROVIDER=gemini` fallback in `model/load.py`): tried this session to route around the Bedrock block — registered the `gemini-api-key` AgentCore Identity credential and wired `MODEL_PROVIDER=gemini` into the deployed runtime env (`cdk-stack.ts`). Confirmed working for plain prompts, but every `clone_repo`/tool-calling invoke failed with a Google 503. Root-caused via `agentcore logs` (the failing calls are all `streamGenerateContent`) plus direct `curl` reproduction: it's not overload, it's `RESOURCE_EXHAUSTED` — this API key's free tier is capped at **20 requests/day for `gemini-3.6-flash`**, and this session's testing already exhausted it. Confirmed `gemini-2.5-flash` is not an option either (`404: no longer available to new users` on this key).
  - Left `MODEL_PROVIDER=gemini` deployed rather than reverting (committed in `7a91970`). Do **not** invoke the agent or open the demo PR again today — every call (including a plain "say hi") burns against the same 20/day cap. **Resume Task 7 once the Gemini daily quota resets (Google resets free-tier daily quotas at midnight Pacific) — retry `agentcore invoke --json "say hi"` first, then `clone_repo`, before attempting the full PR flow.** If 20/day still isn't enough headroom for a full PR review (each review is several tool calls = several Gemini requests), the real fix is upgrading this Gemini key to a paid tier, not more retries.
